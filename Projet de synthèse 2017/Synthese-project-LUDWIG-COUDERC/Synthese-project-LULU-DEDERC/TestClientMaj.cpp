	...
	try
	{
		...
		do
		{
			...
			if (continuer)
			{
				...
				if (r == SOCKET_ERROR)
					throw new exception("échec de l'envoi de la requête");
				...
				do
				{
					...
					strcat(res, reponse);
					if (r == SOCKET_ERROR)
						throw new exception("La réception de la réponse a échoué");
		...
		if (r == SOCKET_ERROR)
			throw new exception("la coupure de connexion a échoué");
		...
		if (r) throw new exception("La fermeture du socket a échoué");
